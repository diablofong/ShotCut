...
### Requirement: 球員標記（編號 + 名字）
系統 SHALL 支援在標記上關聯一或多個球員，每位球員包含背號與名字。

#### Scenario: 關聯多個球員（含名字）
- **WHEN** 使用者在一個標記上輸入「7 林書豪, 11 王大明」
- **THEN** 系統記錄該標記關聯球員 7 號（林書豪）和 11 號（王大明）

#### Scenario: 僅輸入編號
- **WHEN** 使用者只輸入「7, 11」（無名字）
- **THEN** 系統記錄球員 7 號和 11 號，名字為空
...
#### Scenario: API 回傳球員資訊
- **WHEN** 查詢標記列表
- **THEN** 每個標記回傳 `players: [{ number, name }]` 以及 `player_numbers: [int]`（向後相容）
